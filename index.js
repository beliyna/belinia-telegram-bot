...
const sohbetKomutlari = {
  ...
  "sus": () => "susmıycam",
  "şaka": () => "Bilgisayar neden denize girmez? Çünkü suyu sevmez 😅",
  "motivasyon": () => "Başarı, küçük adımların toplamıdır 💪",
  "romantik": () => "Gözlerinin sükûnetinde kaybolmak isterdim 🌹",
  "fal": () => "Bugün yıldızlar senin için gülümsüyor ✨",
  "zihinoku": () => "Şu an aklında ne var? 🤔",
  "itiraf": () => "Sana bir sır vereyim mi? 🤫",
  "dedikodu": () => "Son haber: Beliyna her zaman haklı 😎",
  "övgü": () => "Sen muhteşemsin! 🌟",
  "eğlendir": () => "Hadi gülümse 😆",
  "unut": () => "Tamam, unuttum 😌",
  "sessizlik": () => "Sessizlik moduna geçildi 🔇",
  "iyi geceler": () => "Tatlı rüyalar 😴",
  "günaydın": () => "Günaydın ☀️",
  "iyi akşamlar": () => "Akşamın keyifli olsun 🌙",
  "teşekkür": () => "Rica ederim 🙏",
  "merhaba": () => "Merhaba! 👋",
  "hoşgeldin": () => "Hoşbulduk 😄",
  "aşk": () => "Aşk her zaman güzel 💘",
  "mutluluk": () => "Mutluluk küçük şeylerde gizlidir 🌸",
  "üzgünüm": () => "Üzülme, her şey yoluna girecek 💛",
  "yardım": () => "Ben buradayım, nasıl yardımcı olabilirim? 🤖",
  "merak etme": () => "Merak etme, her şey yolunda 😌",
  "harika": () => "Harika! 😎",
  "hmm": () => "Hmm 🤔",
  "tabii": () => "Tabii ki 🌟",
  "şimdi": () => "Şimdi mi? 😅",
  ...
  "harika fikir": () => "Kesinlikle! 🌟",
  "tekrar": () => "Tamam, tekrar ediyorum 🔄",
  "tam olarak": () => "Tam olarak öyle! ✔️",
...
bot.command('off', ctx => ctx.reply('Bot durduruldu 😴'));
Object.keys(sohbetKomutlari).forEach(cmd => {
    bot.command(cmd, ctx => ctx.reply(sohbetKomutlari[cmd]()));
});
...
bot.on('text', ctx => {
    ...
    // Patron/admin kontrolü
    if (text.includes('sahip') || text.includes('admin')) {
        return ctx.reply('tek sahibim beliyna');
    }

    // Sohbet komutları
    for (let key in sohbetKomutlari) {
        if (text.includes(key)) {
            return ctx.reply(sohbetKomutlari[key]());
        }
    }

    // Tanımlanmayan mesajlar
    ctx.reply('bu komut bünyemde mevcut değil sahibimle iletişime geç');
});
